...
GEOB.getfeatureinfo = (function() {
    ...
    var onBeforegetfeatureinfo = function() {
        ...
        observable.fireEvent("search", {
            html: '<div>'+OpenLayers.i18n('Recherche en cours...')+'</div>'
        });
    };
    ...
    return {
        ...
        toggle: function(record, state) {
            ...
            if (state) {
                observable.fireEvent("search", {
                    html: '<div>'+OpenLayers.i18n('Recherche d\'objets activée sur la couche ')+
                        title+OpenLayers.i18n('. Cliquez sur la carte.')+'</div>'
                });
                ...
                ctrl = new OpenLayers.Control.WMSGetFeatureInfo({
                    layers: [layer],
                    maxFeatures: 10, //GEOB.config.MAX_FEATURES,
