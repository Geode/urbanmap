...
GEOB.managelayers = (function() {
    ...
    var LayerNode = Ext.extend(GeoExt.tree.LayerNode, {
        constructor: function(config) {
            ...
            //console.log('geob_layer : '+config.text+' - '+config.qtip);
    ...
    var actionHandler = function(node, action, evt) {
    	...
    	if (layer.name == UrbanMap.config.layer_buffer || layer.name == 'Parcelles sélectionnées')
    	{
    		//cannot move / delete these layers!
            GEOB.util.infoDialog({
                title: 'Avertissement, couche utilisée par le système',
                msg: 'Impossible de réaliser cette action sur la couche ' + layer.name
                +'<br />Cette couche est nécessaire au bon fonctionnement de l\'application.'
            });
    	}
    	else
    	{
	        ...
	        case "delete":
	            GEOB.util.confirmDialog({
	                msg: OpenLayers.i18n("Voulez-vous réellement supprimer la couche ")+
    ...
    var formatAttribution = function(layerRecord) {
        ...
        return {
            ...
            autoEl: {
                tag: 'span',
                html: ' | '+OpenLayers.i18n('source')+' : '+attrDisplay
            }
        };
    };
    ...
    var formatVisibility = function(layerRecord) {
        ...
        return {
            ...
            autoEl: {
                tag: 'span',
                'ext:qtip': OpenLayers.i18n("Plage de visibilité (indicative) :<br />de ")+visibilityText,
                html: visibilityText
            }
        };
    };
    ...
    var createGfiButton = function(layerRecord) {
        return {
            ...
            enableToggle: true,
            toggleGroup: "tools",
            tooltip: OpenLayers.i18n("Interroger les objets de cette couche"),
            listeners: {
     ...
     *          for inclusion in layer manager item toolbar
     */
    var createGmButton = function(layerRecord) {
        return {
            xtype: 'button',
            disabled: false,
            ...
            allowDepress: false, // false is required here because
                                 ...
                                 // in toggleGroup
            enableToggle: false,
            tooltip: OpenLayers.i18n("Redessiner cette couche"),
            listeners: {
    ...
    var createFormatMenu = function(layerRecord) {
        ...
        if (formats && formats.length) {
            ...
            formatMenuItems.push(new Ext.menu.CheckItem({
                text: 'f?',//layer.params.FORMAT,
                value: 'f?',//layer.params.FORMAT,
                checked: true,
                group: 'format_' + layer.id
            }));
        }
    ...
    var createStylesMenu = function(layerRecord) {
        ...
        var stylesMenuItems = [
            '<b class="menu-title">'+OpenLayers.i18n('Choisissez un style')+'</b>',
            default_style
        ];
    ...
    var createMenu = function(layerRecord) {
        ...
        if (layerRecord.get("metadataURLs") &&
            ...
            menuItems.push({
                ...
                text: OpenLayers.i18n("Fiche de métadonnées"),
                listeners: {
                    "click": function(btn, pressed) {
                        window.open(url);
                    }
        ...
        if (GEOB.styler && queryable) {
            menuItems.push({
                ...
                text: OpenLayers.i18n("Editer la symbologie"),
                listeners: {
                    "click": function(btn, pressed) {
        ...
        if (GEOB.querier && queryable) {
            menuItems.push({
                ...
                text: OpenLayers.i18n("Construire une requête"),
                listeners: {
                    "click": function(btn, pressed) {
        ...
        menuItems.push({
            text: OpenLayers.i18n('Styles prédéfinis'),
            disabled: !queryable,
            menu: stylesMenu
        });
        menuItems.push({
            text: OpenLayers.i18n("Format d'image"),
    ...
    var createLayerNodePanel = function(node, ct) {
        ...
        //buttons.push(createGmButton(layerRecord));
    ...
    return {
        ...
        create: function(layerStore) {
            ...
            layerContainer = new GeoExt.tree.LayerContainer({
                ...
                loader: {
                    baseAttrs: {
                        ...
                        actions: [{
                            action: "delete",
                            qtip: OpenLayers.i18n("supprimer cette couche")
                        }, {
                            action: "up",
                            qtip: OpenLayers.i18n("monter cette couche"),
                            update: function(el) {
            ...
            return {
                ...
                root: layerContainer,
                /*buttons: [{
                    text: OpenLayers.i18n('Ajouter des couches'),
                    handler: function() {
                        if (!layerFinder) {
