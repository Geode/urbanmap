...
GEOB.resultspanel = (function() {
     ...
     *
     * Parameters:
     * columnModel - {Ext.grid.ColumnModel} the grid's column model
     * store - {GeoExt.data.FeatureStore} our feature store
     */
    var createGridPanel = function(columnModel, store) {

        var c = store.getCount();
        var plural = (c>1) ? "s" : "";

        var tbtext = new Ext.Toolbar.TextItem({
            text: (c == 10) ? //GEOB.config.MAX_FEATURES
                ' <span ext:qtip="'+OpenLayers.i18n('Utilisez un navigateur plus performant '+
                'pour augmenter le nombre d\'objets affichables"')+'>'+
                OpenLayers.i18n('Nombre maximum d\'objets atteint')+'('+'10'+')</span>': //GEOB.config.MAX_FEATURES
                c+" résultat"+plural
        });

        var bbar = [{
            text: OpenLayers.i18n('Effacer'),
            tooltip: OpenLayers.i18n("Supprimer les résultats affichés sur la carte et dans le tableau"),
            handler: function() {
            ...
            text: OpenLayers.i18n('Zoom'),
            tooltip: OpenLayers.i18n("Cadrer l'étendue de la carte sur celle des résultats"),
            handler: zoomToLayerExtent
        },{
            text: OpenLayers.i18n('Export CSV'),
            tooltip: OpenLayers.i18n("Exporter l'ensemble des résultats en CSV"),
    ...
    var populate = function(options) {
        ...
        if (!features || features.length === 0) {
            ...
            observable.fireEvent("panel", {
                ...
                html: '<p>'+OpenLayers.i18n('Aucun objet ne correspond à votre requête.')+'</p>'
            });
            return;
        }
